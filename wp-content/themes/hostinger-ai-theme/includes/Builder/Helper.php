...
class Helper {
    ...
    /**
     * Insert a post carrying trusted, first-party block markup.
     *
     * Two things would otherwise corrupt the block markup and make the editor
     * offer "Attempt block recovery":
     *
     * 1. wp_insert_post() unslashes the data, so the serialized content is
     *    slashed here first — otherwise block-comment escapes like
     *    `\u002d\u002d` (from `--` in gradient CSS vars) are stripped.
     * 2. Without the unfiltered_html capability (e.g. under WP-CLI) KSES strips
     *    block attributes / inline styles — a cover image's `data-object-fit`,
     *    gradient/`url()` CSS — desyncing the saved markup from each block's
     *    save() output. KSES is disabled for the insert and restored after,
     *    which is safe because the envelope is our own generated content.
     *
     * @param array<string, mixed> $postarr
     * @return int|\WP_Error
     */
    public static function insert_trusted_post( array $postarr ) {
        kses_remove_filters();
    ...
    /**
     * Lowest-ID administrator, or 0 when the install has none.
     *
     * Generated content needs an owner when there is no current user (WP-CLI
     * without `--user`), and both the CLI entry point and the post adapter
     * need that lookup — so it lives here rather than in both.
     */
    public static function oldest_administrator_id(): int {
        ...
        return ! empty( $administrator_ids ) ? (int) $administrator_ids[0] : 0;
    }
